#!/bin/bash
# Redshift Cyber Security CIS PostgreSQL 11 CENTOS Linux Audit Script
# Use following command to run this scipt 
# chmod +x redshift_cis_postgressql_11_centos_linux.sh
# ./redshift_cis_postgressql_11_centos_linux.sh
#
...
DATENOW=$(date +"%m-%d-%Y")
REPORTHOME=/tmp/redshift/$DATENOW/CIS_POSTGRESQL_11/
REPORT=/tmp/redshift/$DATENOW/CIS_POSTGRESQL_11/report.txt
POSTGRESQL_DEFAULTS_EXTRA_FILE=/tmp/redshift/$DATENOW/CIS_POSTGRESQL_11/.altpgpass
POSTGRESCONFIG=/tmp/redshift/$DATENOW/CIS_POSTGRESQL_11/postgresql.conf

# create folder structure for report output
mkdir -p $REPORTHOME

# Echo timestamp
echo "########### Redshift CIS PostgreSQL 11 $(date) ###########"
...
for OUTPUT in $(find /etc/postgresql/11/main -name postgresql.conf); do
  cp $OUTPUT $REPORTHOME
done

echo "########### Redshift CIS CIS PostgreSQL 11 $(date) ###########" > $REPORT
...
#Audit
whoami >> $REPORT
ls -la ~postgres/11 2>&1 >> $REPORT
/usr/pgsql-11/bin/postgresql-11-check-db-dir ~postgres/11/data
echo $? >> $REPORT
...
## systemctl status postgresql-11|grep 'ago$'
...
#Remediation:
#Please refer to CIS Postgres 11 Benchmark documentation for this exact remediation.
