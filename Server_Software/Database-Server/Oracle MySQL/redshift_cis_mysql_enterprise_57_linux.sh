#!/bin/bash
# Redshift Cyber Security CIS MYSQL Enterprise 5.7 LinuxAudit Script
# Use following command to run this scipt 
# chmod +x redshift_cis_mysql_enterprise_57_linux.sh
# ./redshift_cis_mysql_enterprise_57_linux.sh
#
...
DATENOW=$(date +"%m-%d-%Y")
REPORTHOME=/tmp/redshift/$DATENOW/CIS_MYSQL_ENTERPRISE_5_7/
REPORT=/tmp/redshift/$DATENOW/CIS_MYSQL_ENTERPRISE_5_7/report.txt
MYSQL_DEFAULTS_EXTRA_FILE=/tmp/redshift/$DATENOW/CIS_MYSQL_ENTERPRISE_5_7/mycnf.cnf
...
# Echo timestamp
echo "########### Redshift CIS MYSQL Enterprise 5.7 $(date) ###########"
...
echo "########### Redshift CIS MYSQL Enterprise 5.7 $(date) ###########" > $REPORT
...
echo -e "\r\n----> 2.1 Dedicate Machine Running MySQL (Not Scored)" >> $REPORT
...
#Audit:
#Verify there are no other roles    enabled    for    the    underlying    #operating    systemand    that    no    additional    applications    or    services    unrelated    #to    the    proper    operation    of    the    MySQL    server    software    are    #installed.
#Remediation:
#Remove    excess    applications    or    services    and/or    remove    unnecessary    #roles    from    the    underlying    operating    system
#Impact:
#Care    must    be    taken    that    applications    or    services    that    are    required    for    the    proper    operation    of    the    operating    system    are    not    removed.Custom    applications    may    need    to    be    modified    to    accommodate    database    connections    over    the    network    rather    than    on    the    use    (e.g.,    using    TCP/IP    connections).Additional    hardware    and    operating    system    licenses    may    be    required    to    make    the    architectural    change.


echo -e "\r\nRequires manual verification" >> $REPORT

echo -e "\r\n----> 2.2 Do Not Specify Passwords in Command Line (Not Scored)" >> $REPORT
...
#Remediation:
#Use    -pwithout    password    and    then    enter    the    password    when    prompted,    use    a    properly    secured    .my.cnffile,    or    store    authentication    information    in    encrypted    format    in    .mylogin.cnf.
#Impact:
#Depending    on    the    remediation    chosen,    additional    steps    may    need    to    be    undertaken    like:
#•Entering    a    password    when    prompted;
#•Ensuring    the    file    permissions    on    .my.cnfis    restricted    yet    accessible    by    the    user;
#•Using    mysql_config_editor    to    encrypt    the    authentication    credentials    in    .mylogin.cnf.Additionally,    not    all    scripts/applications    may    be    ableto    use    .mylogin.cnf.
#References:
...
echo -e "\r\n----> 2.3 Do Not Reuse User Accounts (Not Scored)" >> $REPORT
...
echo -e "\r\n----> 2.4 Do Not Use Default or Shared Cryptographic Material (Not Scored)" >> $REPORT

#Description:
#The    cryptographic    material    used    by    MySQL,    such    as    digital    certificates    and    encryption    keys,    should    be    used    only    for    MySQL    and    only    for    one    instance.    Default    cryptographic    material    should    not    be    used    because    others    are    likely    to    have    copies    of    them.
#Rationale:
...
#ps aux |grep mysql >> $REPORT         ------Double check this
...
#To ensure this remediation remains indefinite for the life of the MySQL Server, set
...
#References:
#1. https://dev.mysql.com/doc/refman/5.6/en/audit-log-plugin-options-
#variables.html#sysvar_audit_log_connection_policy
#2. https://dev.mysql.com/doc/refman/5.6/en/audit-log-plugin-logging-control.html
...
echo -e "\r\n----> 9.2 Ensure MASTER_SSL_VERIFY_SERVER_CERT is set to YES or 1" >> $REPORT
