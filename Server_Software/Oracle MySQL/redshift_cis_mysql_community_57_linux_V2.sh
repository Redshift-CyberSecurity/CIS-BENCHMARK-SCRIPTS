#!/bin/bash
# Redshift Cyber Security CIS MYSQL Community 5.7 LinuxAudit Script
# Use following command to run this scipt 
# chmod +x redshift_cis_mysql_community_57_linux.sh
...
# Echo timestamp
echo "########### Redshift CIS MYSQL Community 5.7 $(date) ###########"
...
echo -e "\r\n----> 2.6 Set a Password Expiry Policy for Specific Users" >> $REPORT

#Description:	
#Password expiry for specific users provides user passwords with a unique time bounded lifetime.
#Rationale:
#Allows additional security factors pertinent to a specific user to provide further password security;	
#predetermined by varying security needs and usability requirements in a system or organization [1].
#Remediation:	
#Using the user	and host information from the audit procedure,set each user a password lifetime	
#Impact:	
...
mysql --defaults-extra-file=$MYSQL_DEFAULTS_EXTRA_FILE information_schema -e 'SELECT user, host, password_lifetime FROM mysql.user WHERE (password_lifetime IS NULL); ' >> $REPORT
...
echo -e "\r\n----> 6.3 Ensure log error verbosity is not set to 1" >> $REPORT

#Description:
#The log_error_verbosity system	variable provides additional information to the	MySQL log. A value of 1 enables logging of error messages.
#A value of 2 enables logging of error and warning messages, and a value of 3 enables logging of error, warning and note messages.	
#Rationale:	
...
mysql --defaults-extra-file=$MYSQL_DEFAULTS_EXTRA_FILE information_schema -e 'SHOW GLOBAL VARIABLES LIKE "log_error_verbosity"; ' >> $REPORT
...
echo -e "\r\n----> 7.1 Ensure passwords are not stored in the global config" >> $REPORT
...
echo -e "\r\n----> 7.2 Ensure 'sql_mode' Contains 'NO_AUTO_CREATE_USER'" >> $REPORT
...
echo -e "\r\n----> 7.3 Ensure passwords are set for all user accounts" >> $REPORT
...
echo -e "\r\n----> 7.4 Ensure default password lifetime is less than or equal to 90" >> $REPORT

#Description
#Password expiry provides passwords with a time bounded lifetime
#Rationale:
#This benchmark prevents a password being set for an indefinite period, therefor reducing the time available that a compromised password is known to an attacker
#Remediation
#Level 1 MySQL RDBMS: global policy	
#SET GLOBAL default_password_lifetime=90
#and in	the configuration file:	default_password_lifetime=90 
#As part of Installation and Planning consider set  an	expiry policy for specific users.
#Doing this will take precedence over the setting specified in default_password_lifetime.
#For example:	 
...
mysql --defaults-extra-file=$MYSQL_DEFAULTS_EXTRA_FILE information_schema -e 'SHOW VARIABLES LIKE "default_password_lifetime";' >> $REPORT



echo -e "\r\n----> 7.5 Ensure password complexity is in place" >> $REPORT
...
echo -e "\r\n----> 7.6 Ensure no users have wildcard hostnames" >> $REPORT
...
echo -e "\r\n----> 7.7 Ensure no anonymous accounts exist" >> $REPORT
